...
<template>
  ...
  >
    <div
      v-motion
      class="w-full h-full fixed top-0 left-0 bg-mosque/50"
      :initial="{ opacity: 0 }"
      :enter="{ opacity: 1 }"
      :leave="{ opacity: 0 }"
    ></div>
    <div
      ref="target"
      v-motion
      class="flex flex-col bg-iron rounded-2xl z-40 shadow-2xl max-w-lg w-full"
    ...
    >
      <div class="w-full p-6">
        <slot name="header">
          <h1 class="text-3xl italic text-resolution-blue">{{ title }}</h1>
        </slot>
      </div>
      <div class="w-full p-6 text-2xl">
        <slot name="content">
          <p>{{ message }}</p>
        </slot>
      </div>
